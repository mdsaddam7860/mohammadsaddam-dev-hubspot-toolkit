...
function makeMeetings(client) {
  const base = '/crm/v3/objects/meetings';
  ...
  return {
    createMeeting: (properties) => client.post(base, toPropertiesObject(properties)),
    getMeeting: (id, properties = []) =>
      client.get(`${base}/${id}`, {
        params: {
          properties: Array.isArray(properties) ? properties.join(',') : properties,
        },
      }),
    searchMeeting: (filterGroups = [], properties = [], limit = 50, after = null) =>
      client.post(`${base}/search`, { filterGroups, properties, limit, after }),
    updateMeeting: (id, properties) =>
