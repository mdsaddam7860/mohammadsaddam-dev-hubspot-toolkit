...
function makeTickets(client) {
  const base = '/crm/v3/objects/tickets';
  ...
  return {
    createTicket: (properties) => client.post(base, toPropertiesObject(properties)),
    getTicket: (id, properties = []) =>
      client.get(`${base}/${id}`, {
        params: {
          properties: Array.isArray(properties) ? properties.join(',') : properties,
        },
      }),
    searchTicket: (filterGroups = [], properties = [], limit = 50, after = null) =>
      client.post(`${base}/search`, { filterGroups, properties, limit, after }),
    updateTicket: (id, properties) =>
